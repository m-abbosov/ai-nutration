...
import { useAdminConversations } from '@/admin/shared/api/conversations'
import type { AdminConversationListItemDto } from '@/admin/shared/api/types'
...
export function ConversationsPage() {
  ...
  const columns: DataTableColumn<AdminConversationListItemDto>[] = [
    ...
    { key: 'messageCount', header: t.conversations.colMessages, align: 'right', render: (row) => <span className="adm-mono">{fmtNumber(row.messageCount, lang)}</span> },
    { key: 'createdAt', header: t.conversations.colCreated, render: (row) => <span className="adm-mono text-[11.5px]">{new Date(row.createdAt).toLocaleDateString()}</span> },
    { key: 'updatedAt', header: t.conversations.colUpdated, render: (row) => <span className="adm-mono text-[11.5px]">{new Date(row.updatedAt).toLocaleDateString()}</span> },
  ]
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <FilterBar search={search} onSearchChange={(v) => { setSearch(v); setPage(1) }} searchPlaceholder={t.conversations.searchPlaceholder} />
      </div>
      ...
      {isError ? (
        ...
        <DataTable
          ...
          loading={isLoading}
          onRowClick={canReadContent ? (row) => navigate(`/admin/conversations/${row.id}`) : undefined}
