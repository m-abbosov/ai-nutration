...
export function CalculatorShell({ calcId, inputs, results }: { calcId: string; inputs: ReactNode; results: ReactNode }) {
...
export function ResultHero({
  ...
  return (
    <div className="text-center">
      ...
      <div className="mt-2 text-[46px] font-medium leading-none tracking-[-.04em] [font-variant-numeric:tabular-nums]" style={{ color: tone?.color }}>
        {value}
  ...
  return (
    <div className="animate-fi py-10 text-center">
      <div className="relative mx-auto mb-[18px] h-[64px] w-[64px]">
        <div
          className="animate-halo absolute inset-[-8px] rounded-full blur-[14px]"
          style={{ background: "radial-gradient(circle, var(--accG), transparent 66%)" }}
        />
        <div
          className="absolute inset-[9px] rounded-full"
          style={{
            background: "radial-gradient(circle at 34% 26%, var(--acc), var(--accD) 56%, var(--surf) 96%)",
            boxShadow: "inset 0 0 12px rgba(255,255,255,.24)",
          }}
        />
      </div>
