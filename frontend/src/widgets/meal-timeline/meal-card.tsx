      ...
      <div className="min-w-0 flex-1">
        <div className="flex cursor-pointer items-center gap-3" onClick={() => setOpen((v) => !v)}>
          ...
          <span className="min-w-0 flex-1">
            <span className="block truncate text-[14px] font-semibold tracking-[-.012em] md:text-[15px]">
              {meal.name}
            </span>
          ...
          <span className="flex flex-none items-baseline gap-1">
            <span className="text-[17px] font-medium tracking-[-.02em] tabular-nums md:text-[19px]">
              {fmtNumber(meal.calories, lang)}
            </span>
          ...
          <span className="ml-1.5 flex flex-none gap-0.5" onClick={(e) => e.stopPropagation()}>
            ...
            >
              <svg width="14" height="14" viewBox="0 0 16 16">
                <path
                  d="M3 12.4 11.1 4.3l1.6 1.6L4.6 14H3v-1.6Z"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="1.3"
        ...
        {open && (
          <div className="mt-3.5 animate-fu">
            {meal.items.length > 0 ? (
              ...
              <div className="rounded-2xl border border-dashed border-line2 p-4 text-[13px] text-tx3">
                {meal.servingSize ?? '—'}
              </div>
            )}
          </div>
