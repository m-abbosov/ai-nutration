...
export function NutritionResultCard({ analysis, added, onAdded }: { analysis: NutritionAnalysisDto; added: boolean; onAdded: () => void }) {
  ...
  return (
    <div className="mt-3.5 max-w-[400px] overflow-hidden rounded-[18px] border border-line bg-surf shadow-card">
      <div className="flex items-center gap-2.5 border-b border-line px-4 pb-3.5 pt-[15px]">
        <span className="grid h-9 w-9 flex-none place-items-center rounded-xl bg-accT text-[17px]">{MEAL_TYPE_EMOJI[analysis.mealType]}</span>
        ...
        <div className="text-right">
          <div className="text-[21px] font-medium leading-none tracking-[-.025em] tabular-nums">{fmtNumber(analysis.total.calories, lang)}</div>
