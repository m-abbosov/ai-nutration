...
export function calculateBodyFat(gender: Gender, heightCm: number, neckCm: number, waistCm: number, hipCm?: number): BodyFatResult {
...
export function calculateLeanBodyMass(gender: Gender, heightCm: number, weightKg: number): LeanBodyMassResult {
  const isFemale = gender === "FEMALE";
  const boerKg = isFemale ? 0.252 * weightKg + 0.473 * heightCm - 48.3 : 0.407 * weightKg + 0.267 * heightCm - 19.2;
  const jamesKg = isFemale ? 1.07 * weightKg - 148 * Math.pow(weightKg / heightCm, 2) : 1.1 * weightKg - 128 * Math.pow(weightKg / heightCm, 2);
  const humeKg = isFemale ? 0.29569 * weightKg + 0.41813 * heightCm - 43.2933 : 0.3281 * weightKg + 0.33929 * heightCm - 29.5336;
