...
export function AdminSkeleton({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("adm-animate-pulse rounded-[var(--adm-radius-sm)]", className)} style={{ background: "var(--adm-bg-inset)" }} {...props} />
  );
}
    ...
    <div className="rounded-[var(--adm-radius-lg)] border" style={{ background: "var(--adm-surface)", borderColor: "var(--adm-border)" }}>
      ...
      <div>
        {Array.from({ length: rows }).map((_, r) => (
          <div key={r} className="flex items-center gap-4 border-b p-3 last:border-b-0" style={{ borderColor: "var(--adm-border)" }}>
