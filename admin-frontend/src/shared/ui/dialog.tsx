...
export function AdminDialogTitle({ className, ...props }: ComponentPropsWithoutRef<typeof DialogPrimitive.Title>) {
  return (
    <DialogPrimitive.Title className={cn("text-[15px] font-semibold tracking-tight", className)} style={{ color: "var(--adm-text)" }} {...props} />
  );
}

export function AdminDialogDescription({ className, ...props }: ComponentPropsWithoutRef<typeof DialogPrimitive.Description>) {
  return <DialogPrimitive.Description className={cn("text-[12.5px] leading-relaxed", className)} style={{ color: "var(--adm-text-2)" }} {...props} />;
}
